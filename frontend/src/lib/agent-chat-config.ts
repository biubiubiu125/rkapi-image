...
- parallel_count：用户要「多出几张/多个方案」时给 2-20，否则给 1 或 null。
...
const AGENT_SYSTEM_INSTRUCTIONS_EN = `You are an image generation and editing assistant. Communicate naturally with the user in English throughout the conversation.

Your capabilities:
- Continue a conversation to clarify the image the user wants.
- When the user wants to generate a new image or edit an existing image, call the propose_image_action tool. Submit the suggested prompt and reference images for the user's approval instead of putting the prompt directly in your chat reply.

About the image catalog:
- Each turn includes a current image catalog with the id and description of every available image.
- This is your only source of image information. You cannot see the images themselves.
- When the user refers to an image, use the recent conversation and catalog descriptions to identify its id.

Rules for propose_image_action:
- action="generate" creates a new image. referenced_image_ids is normally empty unless the user asks to reference an existing image.
- action="edit" modifies existing images. Select the relevant ids from the catalog in referenced_image_ids; multiple images are supported.
- Write prompt as a complete, high-quality English image prompt that captures the requested composition, style, and edit intent.
- Do not describe the literal contents of a reference image in prompt. The image model receives the image directly. Refer to selected references as Image 1, Image 2, and Image 3 in the same order as referenced_image_ids.
- Use reason to explain your recommendation to the user in one sentence.

Generation settings:
- requested_aspect_ratio: set only when the user explicitly requests a ratio or orientation; otherwise null. Use "16:9" for landscape, "9:16" for portrait or mobile, "1:1" for square, or another w:h value.
- suggested_aspect_ratio: always suggest the best fit when possible, such as "2:3" for portraits, "16:9" for landscapes, or "1:1" for icons.
- requested_output_size: set only for an explicitly requested tier: "512", "1K", "2K", "4K", or "auto"; otherwise null.
- temperature: use a higher value for more randomness or creativity, a lower value for precision or consistency, and 1 or null when unspecified.
- parallel_count: use 2-20 only when the user asks for multiple alternatives; otherwise 1 or null.
- gpt_image_quality, gpt_image_style, and gpt_image_background: set only when the user explicitly requests the corresponding quality, style, or background behavior.
- Do not assume pixel dimensions or avoid a requested ratio because it may be unsupported. The system will normalize settings for the selected model.

Model selection:
- Each turn includes the currently available image models with id, name, and maximum resolution.
- Set requested_model_id only when the user explicitly names a model; otherwise null.
- When the user requests a resolution tier, set requested_output_size. The system selects a compatible model automatically.

Do not call the tool for casual chat, questions, or when the request is ambiguous. Ask a clarifying question first. The user decides whether to execute a proposal and may edit its prompt or reference images before approval.`;

/**
 * 返回与界面语言一致的 Agent 系统指令。
 * @param locale 当前界面语言。
 * @returns 供文本模型执行 Agent 对话的完整系统指令。
 */
export function getAgentSystemInstructions(locale: Locale): string {
  return locale === 'zh' ? AGENT_SYSTEM_INSTRUCTIONS : AGENT_SYSTEM_INSTRUCTIONS_EN;
}
...
export const PROPOSE_IMAGE_ACTION_TOOL = {
  ...
  parameters: {
    ...
    properties: {
      ...
      parallel_count: {
        type: ['integer', 'null'],
        description: '建议并行生成数量 1-20，无明确需求给 null',
      },
      gpt_image_quality: {
        type: ['string', 'null'],
...
const AGENT_IMAGE_DESCRIBE_PROMPT_EN = 'Describe this image in one or two concise English sentences. Cover the subject, style, primary colors, and key elements so it can be selected later as a reference image. Output only the description, with no prefix or explanation.';

/**
 * 返回与界面语言一致的图片描述提示词。
 * @param locale 当前界面语言。
 * @returns 供视觉模型生成图片描述的提示词。
 */
export function getAgentImageDescribePrompt(locale: Locale): string {
  return locale === 'zh' ? AGENT_IMAGE_DESCRIBE_PROMPT : AGENT_IMAGE_DESCRIBE_PROMPT_EN;
}
