...
export async function checkExistingTask(taskId: string, readToken?: string): Promise<{ status: FlyreqTaskResponse["status"]; images?: CanvasGeneratedImage[]; error?: string }> {
  const task = await getFlyreqTask(taskId, readToken);
